...
abstract class AbstractVal[T] (implicit tring: Ring[T]) {
...
case class Label[T](name : String)(implicit num : Ring[T]) extends AbstractVal[T] {
...
class AbsRing[T](implicit tring : Ring[T]) extends Ring[AbstractVal[T]] {
...
object AbstractVal {
  ...
  def parse[T] (s : String)(implicit tring: Ring[T]): AbstractVal[T] = AbstractVal[T] ({
  ...
  def eval[T]( m: Monomial[T], feed: Label[T] => T )(implicit tring : Ring[T]) : T =
    (m map tupled { (label, power) => feed(label) ** power }).reduce( tring.times )

  def apply[T : Ring] (m: Multinomial[T]) : AbstractVal[T] = new AbstractVal[T] { override val poly = m }

  implicit def multinomial_of_absval[T : Ring](a : AbstractVal[T]) : Multinomial[T] = a.poly
  implicit def absval_of_multinomial[T : Ring](m : Multinomial[T]) : AbstractVal[T] = apply(m)
}
